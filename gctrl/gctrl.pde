...
void draw()
{
  ...
  text("page up: pen up", 12, y); y += dy;
  text("page down: pen down", 12, y); y+= dy;
...
void keyPressed()
{
  if (key == '1') speed = 0.0254;
  if (key == '2') speed = 0.254;
  if (key == '3') speed = 2.54;
  ...
  if (!streaming) {
    ...
    if (keyCode == LEFT) port.write("G91 X" + speed + "\n");
    if (keyCode == RIGHT) port.write("G91 X-" + speed + "\n");
    if (keyCode == UP) port.write("G91 Y-" + speed + "\n");
    if (keyCode == DOWN) port.write("G91 Y" + speed + "\n");
    if (keyCode == KeyEvent.VK_PAGE_UP) port.write("M300 S50\n");
    if (keyCode == KeyEvent.VK_PAGE_DOWN) port.write("M300 S30\n");
    if (key == 'h') port.write("G00 X0 Y0\n");
    ...
    if (key == '0') port.write("G92 X0.00 Y0.00\n");
